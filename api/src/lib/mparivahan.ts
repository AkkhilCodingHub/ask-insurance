...
// ── Real Live Vehicle Detail Fetcher (No Synthetic/Fake Generation) ───────────
export async function getVehicleRcDetails(regNumber: string): Promise<MParivahanVehicleDetails | null> {
  const norm = regNumber.toUpperCase().replace(/[^A-Z0-9]/g, '');
  // Only return authentic results from live official mParivahan API gateway
  return await fetchFromMParivahanApi(norm);
}
